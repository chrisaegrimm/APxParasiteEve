...
def connect_regions(world: PE1World) -> None:
    ...
    manhattan.connect(ch_norq, "World Map to CH:NoRq")
    manhattan.connect(pd_norq, "World Map to PD:NoRq")
    manhattan.connect(cp_norq, "World Map to CP:NoRq")
    manhattan.connect(sh_norq, "World Map to SH:NoRq")
    manhattan.connect(hs_norq, "World Map to HS:NoRq")
    manhattan.connect(ct_norq_or_cmbt_gk, "World Map to CT:NoRq|Cmbt+GK")
    manhattan.connect(sw_cmbt_pk_or_gk, "World Map to SW:Cmbt+PK|GK")
    manhattan.connect(wh_norq, "World Map to WH:NoRq")
    manhattan.connect(mu_norq, "World Map to MU:NoRq")
    manhattan.connect(cb_k1, "World Map to CB:K1")

    ch_norq.connect(ch_cmbt, "CH:NoRq to CH:Cmbt")
    ch_cmbt.connect(ch_cmbt_tk, "CH:Cmbt to CH:Cmbt+TK")
    ch_cmbt.connect(ch_cmbt_rk, "CH:Cmbt to CH:Cmbt+RK")
    ch_cmbt_rk.connect(ch_boss_rk, "CH:Cmbt+RK to CH:Boss+RK")

    pd_norq.connect(pd_lk, "PD:NoRq to PD:LK")
    pd_norq.connect(pd_sk, "PD:NoRq to PD:SK")
    pd_norq.connect(pd_sj, "PD:NoRq to PD:SJ")
    pd_sj.connect(pd_cmbt_sj, "PD:SJ to PD:Cmbt+SJ")
    pd_cmbt_sj.connect(pd_boss_sj, "PD:Cmbt+SJ to PD:Boss+SJ")
    pd_boss_sj.connect(pd_boss_sj_junk, "PD:Boss+SJ to PD:Boss+SJ+Junk")
    pd_boss_sj.connect(pd_boss_sj_12rt, "PD:Boss+SJ to PD:Boss+SJ+12RT")
    pd_boss_sj_12rt.connect(pd_boss_sj_14rt, "PD:Boss+SJ+12RT to PD:Boss+SJ+14RT")

    cp_norq.connect(cp_cmbt, "CP:NoRq to CP:Cmbt")
    cp_cmbt.connect(cp_cmbt_zk, "CP:Cmbt to CP:Cmbt+ZK")
    cp_cmbt_zk.connect(cp_boss_zk, "CP:Cmbt+ZK to CP:Boss+ZK")

    sh_norq.connect(sh_wm, "SH:NoRq to SH:WM")

    hs_norq.connect(hs_gc, "HS:NoRq to HS:GC")
    hs_norq.connect(hs_bc, "HS:NoRq to HS:BC")
    hs_norq.connect(hs_cmbt, "HS:NoRq to HS:Cmbt")
    hs_cmbt.connect(hs_cmbt_ak, "HS:Cmbt to HS:Cmbt+AK")
    hs_gc.connect(hs_cmbt_gc, "HS:GC to HS:Cmbt+GC")
    hs_bc.connect(hs_cmbt_bc, "HS:BC to HS:Cmbt+BC")
    hs_cmbt_bc.connect(hs_cmbt_bc_3f, "HS+Cmbt+BC to HS:Cmbt+BC+3F")
    hs_cmbt_bc_3f.connect(hs_boss_bc_3f_ek, "HS:Cmbt+BC+3F to HS:Boss+BC+3F+EK")

    ct_norq_or_cmbt_gk.connect(ct_cmbt_or_cmbt_gk, "CT:NoRq|Cmbt+GK to CT:Cmbt|Cmbt+GK")
    ct_cmbt_or_cmbt_gk.connect(ct_boss_or_boss_gk, "CT:Cmbt|Cmbt+GK to CT:Boss|Boss+GK")
    ct_cmbt_or_cmbt_gk.connect(ct_cmbt_or_gk, "CT:Cmbt|Cmbt+GK to CT:Cmbt|GK")
    ct_cmbt_or_gk.connect(sw_cmbt_pk_or_gk, "CT:Cmbt|GK to SW:Cmbt+PK|GK")

    sw_cmbt_pk_or_gk.connect(sw_cmbt_pk_or_cmbt_gk, "SW:Cmbt+PK|GK to SW:Cmbt+PK|Cmbt+GK")
    sw_cmbt_pk_or_cmbt_gk.connect(sw_boss_pk_or_boss_gk, "SW:Cmbt+PK|Cmbt+GK to SW:Boss+PK|Boss+GK")

    wh_norq.connect(wh_cmbt, "WH:NoRq to WH:Cmbt")
    wh_cmbt.connect(wh_cmbt_wk, "WH:Cmbt to WH:Cmbt+WK")
    wh_cmbt_wk.connect(wh_boss_wk, "WH:Cmbt+WK to WH:Boss+WK")

    mu_norq.connect(mu_cmbt, "MU:NoRq to MU:Cmbt")
    mu_cmbt.connect(mu_cmbt_kk, "MU:Cmbt to MU:Cmbt+KK")
    mu_cmbt_kk.connect(mu_boss_kk, "MU:Cmbt+KK to MU:Boss+KK")
    mu_boss_kk.connect(liberty, "MU:Boss+KK to SL:Boss+3C")
    liberty.connect(cruiser, "SL:Boss+3C to CR:Boss+3C")

    cb_k1.connect(cb_k2, "CB:K1 to CB:K2")
    cb_k2.connect(cb_k3, "CB:K2 to CB:K3")
    cb_k3.connect(cb_k4, "CB:K3 to CB:K4")
    cb_k4.connect(cb_k5, "CB:K4 to CB:K5")
    cb_k5.connect(cb_k6, "CB:K5 to CB:K6")
    cb_k6.connect(cb_k7, "CB:K6 to CB:K7")
    cb_k7.connect(cb_spire, "CB:K7 to CB:Boss+3C")